...
ZAKAZANE = [
    ...
    (r"—", "česká typografie: pomlčka je „–“, ne anglická „—“"),
    (r"(?<![\w,.])\d+(?:,\d+)?\s*(?:kWp|MWp|kW|MW)?\s*[–—]\s*(?:pod\s+|nad\s+)?\d+(?:,\d+)?\s*(?:kWp|MWp|kW|MW)\b",
     "rozsah výkonu piš slovy, např. „30 MW až pod 75 MW“"),
    (r"\bPn\b", "jmenovitý výkon s dolním indexem: P<sub>n</sub>"),
